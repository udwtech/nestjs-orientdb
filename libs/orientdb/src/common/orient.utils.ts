...
export const getOrientClassToken =
    (orientClass: string): string =>
        `${orientClass}Token`
...
export const getConnectionToken =
    (name: string = DEFAULT_DB_CONNECTIONTOKEN): string =>
        name !== DEFAULT_DB_CONNECTIONTOKEN
            ? `${name}`
            : DEFAULT_DB_CONNECTIONTOKEN
...
export const handleRetry =
    (retryAttempt = 9, retryDelay = 3000): (<T>(source: Observable<T>) => Observable<T>) => {
                ...
                retryWhen(e =>
                    e.pipe(
                        ...
                        scan((errorCount, error) => {
                            ...
                            logger.error(`Unable to connect to database. Retrying (${errorCount + 1})...`, '');
